...
<Button onClick={handleCreateAppointment} className="bg-blue-600 hover:bg-blue-700 text-white">
  {editMode ? "Update Appointment" : "Create Appointment"}
          ...
          <Dialog open={isAllDialogOpen} onOpenChange={setIsAllDialogOpen}>
            ...
            <DialogContent className="w-full max-w-[120vw] max-h-[90vh] overflow-y-auto bg-slate-900 text-white rounded-xl shadow-xl border border-slate-700 p-6">
  <DialogHeader>
    <DialogTitle className="text-2xl font-bold">All Appointments</DialogTitle>
    <DialogDescription className="text-slate-300">
      View, edit, or delete all scheduled appointments below.
    </DialogDescription>
  </DialogHeader>

  <div className="mt-6 grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
    {appointments.length === 0 ? (
      <p className="text-slate-400 text-sm">No appointments available.</p>
    ) : (
      appointments
        .slice()
